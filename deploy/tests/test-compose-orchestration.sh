...
wait_http "http://127.0.0.1:${HTTP_PORT}/api/health" || {
  compose ps -a
  compose logs --tail=120 proxy backend
  exit 1
}
wait_http "http://127.0.0.1:${HTTP_PORT}/" || {
  compose ps -a
  compose logs --tail=120 proxy frontend
  exit 1
}
