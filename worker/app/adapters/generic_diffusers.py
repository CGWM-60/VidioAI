...
class GenericDiffusersAdapter(RuntimeAdapter):
    """Adapter generique guide par la signature reelle de la pipeline."""
    ...
    def estimate_resources(self, metadata: dict[str, Any]) -> dict[str, Any]:
        ...
        size = 20 if "video" in class_name or "wan" in class_name else 10
        return {
    ...
    def generate(
        ...
        values = profile.normalize(request, video=is_video)

        # Le prompt passe tel quel.
        kwargs: dict[str, Any] = {
        ...
        if capability not in IMAGE_CONDITIONED_CAPABILITIES:
            kwargs["width"] = values["width"]
            kwargs["height"] = values["height"]
        else:
            kwargs["width"] = request.get("width")
            kwargs["height"] = request.get("height")

        # requested_fps est un contrat de fichier final, pas un parametre modele.
        ...
        if capability in {
            ...
            end_index = next(
                (index for index, role in enumerate(roles) if role in {"end", "end_frame"}),
                None,
            )
