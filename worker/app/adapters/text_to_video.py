...
class TextToVideoAdapter(RuntimeAdapter):
    ...
    def supports_model(self, metadata: dict[str, Any]) -> bool:
        ...
        return (
            ...
            or "video" in str(metadata.get("pipeline_tag") or "").lower()
        )
    ...
    def _is_wan(runtime: dict[str, Any]) -> bool:
        ...
        class_name = str(
            metadata.get("class_name") or runtime.get("class_name") or ""
        ).lower()
        architectures = [
            str(value).lower()
    ...
    def load(self, snapshot: str, settings: dict[str, Any], runtime: Any) -> Any:
        ...
        from diffusers import AutoPipelineForText2Video, DiffusionPipeline
        try:
    ...
    def generate(
        ...
        log_diffusers_call(pipeline, filtered, str(request.get("capability") or "TEXT_TO_VIDEO"))
